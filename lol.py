...
def knn(x_train, y_train, x, k):
    metrics = np.square(x_train - x).sum(1)
    index = np.argsort(metrics)[:k]
    unique_classes = np.unique(y_train)
    votes = [(y_train[index] == y).sum() for y in unique_classes]
    return np.argmax(votes)

predict = [knn(x_train, y_train, x, 5) for x in x_test]
